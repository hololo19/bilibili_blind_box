...
async def message_listener(ws):
    ...
    try:
        while not ws.closed:
            try:
                ...
                if msg.type == aiohttp.WSMsgType.BINARY:
                    ...
                    # 每100条消息打印一次统计
                    if msg_count % 100 == 0:
                        print(f"[调试] 已接收 {msg_count} 条消息")

                    # 前10条消息打印原始数据用于调试
                    if msg_count <= 10:
                        ver_debug, op_code_debug, body_debug = decode_packet(msg.data)
                        body_data_debug = decompress_body(body_debug, ver_debug)
                        print(f"[调试消息#{msg_count}] ver={ver_debug}, op_code={op_code_debug}, 数据长度={len(body_data_debug)}")
                        if body_data_debug:
                            preview = body_data_debug[:300].decode('utf-8', errors='ignore')
                            print(f"[调试消息#{msg_count}] 内容预览: {preview}...")
                            ...
                            try:
                                ...
                                while pos < len(json_text):
                                    ...
                                    try:
                                        ...
                                        # 输出新发现的命令类型
                                        if cmd and cmd_count.get(cmd) == 1:
                                            print(f"[命令] {cmd}")

                                        # 处理礼物相关命令
                                        if 'GIFT' in cmd or 'gift' in cmd or 'BLIND' in cmd or 'blind' in cmd:
                                            print(f"\n[检测到相关命令] {cmd}")
                                            print(f"[数据] {json.dumps(json_data, ensure_ascii=False)[:800]}...")

                                            blind_gift = json_data.get('data', {}).get('blind_gift')

                                            if blind_gift:
                                                print(f"\n{'='*50}")
                                                print(f"[盲盒]")
                                                print(f"{'='*50}")
                                                print(f"发送者: {json_data.get('data', {}).get('uname', '未知')}")
                                                print(f"盲盒名称: {blind_gift.get('original_gift_name', '未知')}")
                                                blind_price = blind_gift.get('original_gift_price', 0)
                                                print(f"爆出礼物: {json_data.get('data', {}).get('gift_name', '未知')}")
                                                gift_price = blind_gift.get('gift_tip_price', 0)
                                                profit = (gift_price - blind_price) / 1000
                                                print(f"盲盒价格: {blind_price/1000:.2f} 元")
                                                print(f"爆出价值: {gift_price/1000:.2f} 元")
                                                print(f"盈亏: {profit:+.2f} 元")
                                                print(f"{'='*50}\n")

                                    except json.JSONDecodeError as e:
                                        # 如果JSON解析失败，打印错误（仅前10条消息）
                                        if msg_count <= 10:
                                            print(f"[JSON错误#{msg_count}] {str(e)[:50]}")
        ...
        print(f"\n[统计] 总共接收 {msg_count} 条消息")
        print(f"[统计] 命令类型统计:")
        for cmd, count in sorted(cmd_count.items(), key=lambda x: x[1], reverse=True)[:20]:
            print(f"       {cmd}: {count}")
    except Exception:
        pass
    finally:
        # 确保无论如何都显示统计
        if msg_count > 0:
            print(f"\n[最终统计] 总共接收 {msg_count} 条消息")
            print(f"[最终统计] 命令类型统计 (Top 20):")
            for cmd, count in sorted(cmd_count.items(), key=lambda x: x[1], reverse=True)[:20]:
                print(f"           {cmd}: {count}")
